/**
 * Cloudflare Worker entry — the remote Ohio state law MCP server.
 *
 * Routes:
 ...
 * module-level value is the corpus — Ohio Revised Code, Administrative Code,
 * and Constitution sections plus the annotation layer, parsed and validated
 * once per isolate, including the substring guarantee on every quote-safe
 * excerpt.
 *
 * Six tools per server: four oh_* tools and the search/fetch pair for
...
import {
  OhAdapter,
  OhCorpus,
  registerOhConnectorTools,
  registerOhTools,
  type OhItem,
} from '@repairmcp/state-oh';
import corpusJson from '@repairmcp/state-oh/data/oh-law-corpus.json';
import annotationsJson from '@repairmcp/state-oh/data/oh-annotations.json';
...
const SERVER_NAME = 'RepairMCP Ohio Law';
...
async function handleMcp(request: Request): Promise<Response> {
  ...
  const server = new RepairMCPServer<OhItem>(adapter, {
    name: SERVER_NAME,
    version: SERVER_VERSION,
  });

  registerOhTools(server, corpus);
  registerOhConnectorTools(server, adapter, corpus);
      ...
      try {
        ...
        return json({
          ...
          corpus: {
            ...
            newestEffectiveDate:
              (corpus.meta as { newestEffectiveDate?: string }).newestEffectiveDate ?? null,
            ...
            statusNotes: corpus.sections
              .filter((s) => s.statusNote)
              .map((s) => `${s.code} ${s.cite}: ${s.statusNote}`),
          },
        });
      } catch (err) {
      ...
      return withCors(
        new Response(
          `${SERVER_NAME} ${SERVER_VERSION}\n\n` +
            ...
            `Source: Ohio state law for collision repair facilities — insurance claims\n` +
            `handling (the insurer pays the difference or names a shop that will repair for\n` +
            `its written estimate, no unreasonable travel to a specific shop, restoration to\n` +
            `pre-loss condition when the insurer designates a shop, itemized betterment, the\n` +
            `total loss valuation methods, notice before storage payments stop, the 15-day\n` +
            `acknowledgment and 21-day decision deadlines; aftermarket crash parts\n` +
            `disclosure; salvage titles), the Attorney General's motor vehicle repair rule\n` +
            `(the estimate form, authorization for additional work, no charge for\n` +
            `unauthorized work, parts return, the itemized invoice), unclaimed and abandoned\n` +
            `vehicles, employment rules, and Ohio's own safety layer (the VSSR statute and\n` +
            `the BWC specific safety requirements), captured verbatim from codes.ohio.gov.\n` +
