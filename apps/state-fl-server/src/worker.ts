/**
 * Cloudflare Worker entry — the remote Florida state law MCP server.
 *
 * Routes:
 ...
 * module-level value is the corpus — statute and FAC sections plus the
 * annotation layer, parsed and validated once per isolate, including the
 * substring guarantee on every quote-safe excerpt.
 *
 * Six tools per server: four fl_* tools and the search/fetch pair for
...
import {
  FlAdapter,
  FlCorpus,
  registerFlConnectorTools,
  registerFlTools,
  type FlItem,
} from '@repairmcp/state-fl';
import corpusJson from '@repairmcp/state-fl/data/fl-law-corpus.json';
import annotationsJson from '@repairmcp/state-fl/data/fl-annotations.json';
...
const SERVER_NAME = 'RepairMCP Florida Law';
...
async function handleMcp(request: Request): Promise<Response> {
  ...
  const server = new RepairMCPServer<FlItem>(adapter, {
    name: SERVER_NAME,
    version: SERVER_VERSION,
  });

  registerFlTools(server, corpus);
  registerFlConnectorTools(server, adapter, corpus);
      ...
      try {
        ...
        return json({
          ...
          corpus: {
            ...
            // Florida statutes carry the annual edition, not effective dates
            // — the edition phrase Online Sunshine printed at capture, pinned
            // by FL_STATUTES_EDITION. FAC rules carry their own dates.
            statutesEdition:
              (corpus.meta as { statutesEdition?: string }).statutesEdition ?? null,
      ...
      return withCors(
        new Response(
          `${SERVER_NAME} ${SERVER_VERSION}\n\n` +
            ...
            `Source: Florida state law for collision repair facilities — insurance\n` +
            `claims handling (motor vehicle claim settlement practices, the civil\n` +
            `remedy for bad faith, the unfair claims catalog, the 80 percent total loss\n` +
            `threshold, adjuster ethics), the Motor Vehicle Repair Act, liens on\n` +
            `vehicles, aftermarket crash parts, the Deceptive and Unfair Trade\n` +
            `Practices Act, and employment rules, captured verbatim from the\n` +
            `Legislature's Online Sunshine (leg.state.fl.us) and the Florida\n` +
            `Administrative Code (flrules.org).\n` +
